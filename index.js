...
async function fetchSaleDetails (saleAggregatorNumber) {
  ...
  const payload = { saleAggregatorNumber };
...
async function fetchCardSaleList ({ secondaryName, secondaryId }) {
  ...
  while (hasMore) {
    ...
    try {
      ...
      console.error(`Error fetching sale list for card ${secondaryName} on page ${pageNumber}: `, error.message);
      hasMore = false;
    }
  }
